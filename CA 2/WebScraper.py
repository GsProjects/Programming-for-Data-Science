...
def check_table_existance():
    ...
    cursor = conn.cursor()
    query = ('DROP TABLE IF EXISTS Presidents;')
    query1 = ('DROP TABLE IF EXISTS Vacant;')
    query2 = ('DROP TABLE IF EXISTS Vice_President;')
    ...
    if len(president_rows) > 0:
        for item in president_rows:
            ...
            if type(item) is not bs4.element.NavigableString:
                ...
                text = text.replace("National Union April 15, 1865 – c.\u20091868",
                                    'National Union')  # special case for andrew johnson
...
def parse_td_data(data):
    ...
    if len(data) > 8:
        ...
        for items in data:
            ...
            if len(items.get_text()) >= 23 and len(
                    items.get_text()) <= 24:  # special case for when the term data is longer than normal e.g Abraham Lincoln
                data.remove(items)
...
def prep_data(data):
    ...
    presidency_dates = data[0].split(
        '–')  # possible encoding error with the hyphen as the mac hyphen will not work in the split function
    ...
    for items in status:
        for elements in presidency_dates[1].split():
            if items == elements:
                ...
                presidency_dates[1] = presidency_dates[1][
                                      :presidency_dates[1].index(items)]  # get the date up until the status

                new_data.append({'Presidency_Start': datetime.strptime(presidency_dates[0].strip(' ').replace(',', ''),
                                                                       '%B %d %Y').date()})
                new_data.append({'Presidency_End': datetime.strptime(presidency_dates[1].strip(' ').replace(',', ''),
                                                                     '%B %d %Y').date()})
    ...
    if len(new_data) == 0:
        if presidency_dates[1].strip(' ') == 'Incumbent':
            new_data.append({'Presidency_Start': datetime.strptime(presidency_dates[0].strip(' ').replace(',', ''),
                                                                   '%B %d %Y').date()})
            new_data.append(
                {'Presidency_End': datetime.strptime(str(datetime.today().strftime('%B %d %Y')), '%B %d %Y').date()})
...
def vp_info(data, start_vacancy, end_vacancy, information):
    ...
    elif len(data.split()) > 1 and len(
            data.split()) <= 4:  # if theres only one vice president, len of <=4 for people like George H. W. Bush
        new_data.append({'Vice_President_Name': data})
    elif len(
            data.split()) > 0 and '–' not in data.split():  # if the vice president had one of the statuses after their name with no date
        for items in status:
            ...
            if elements == '–':
                ...
                if first_vp_data.split()[hyphen_index - 3] in calendar.month_name:
                    ...
                    if vp_name == 'Office vacant':
                        ...
                        new_data.append({'Vacant': vp_name})
                        new_data.append({'Vacant_Start': datetime.strptime(start_date.strip(' ').replace(',', ''),
                                                                           '%B %d %Y').date()})
                        new_data.append(
                            {'Vacant_End': datetime.strptime(end_date.strip(' ').replace(',', ''), '%B %d %Y').date()})
                        ...
                        if vp_name != 'Unaffiliated':
                            ...
                            new_data.append({'Vice_President_Name': vp_name})
                            new_data.append({'Vice_President_Start': datetime.strptime(
                                start_date.strip(' ').replace(',', ''), '%B %d %Y').date()})
                            new_data.append({'Vice_President_End': datetime.strptime(
                                end_date.strip(' ').replace(',', ''), '%B %d %Y').date()})
                    ...
                    if vp_name == 'Office vacant':
                        ...
                        new_data.append({'Vice_President_Start': datetime.strptime(
                            start_date.strip(' ').replace(',', ''), '%B %d %Y').date()})
                        new_data.append({'Vice_President_End': datetime.strptime(end_date.strip(' ').replace(',', ''),
                                                                                 '%B %d %Y').date()})
    return new_data
...
def parse_new_data(data, president_name):
    ...
    if len(data) == 2:
        ...
        if data[0].get_text().split()[-1] == 'U.S.' or data[0].get_text().split()[-1] == 'U.S.)' or \
                        data[0].get_text().split()[-1] == 'U.S.),':
...
def question_3():
    ...
    cursor = conn.cursor()
    query = (
    'Select President_Name as Presidents_Who_Were_Vice_Presidents from Presidents where President_Name IN (select Vice_President_Name from Vice_President);')
...
def question_4():
    ...
    cursor = conn.cursor()
    query = (
    'select (sum(TIMESTAMPDIFF(month,Vacant_Start,Vacant_End))-mod(sum(TIMESTAMPDIFF(month,Vacant_Start,Vacant_End)),12))/12 as years, mod(sum(TIMESTAMPDIFF(month,Vacant_Start,Vacant_End)),12) as months  from Vacant;')
    cursor.execute(query, )
    result = cursor.fetchone()
    cursor.close()
    conn.close()
...
def question_6():
    ...
    cursor = conn.cursor()
    query = (
    'select President_Name, President_Age from Presidents where DATEDIFF(Presidency_End,President_Birth) = (Select max(DATEDIFF(Presidency_End,President_Birth )) from Presidents);')
    cursor.execute(query, )
    result = cursor.fetchall()
    cursor.close()
    conn.close()
...
def question_7():
    ...
    cursor = conn.cursor()
    query = (
    'select President_Name, max(DATEDIFF(Presidency_End, Presidency_Start)) as Length_Of_Term from Presidents where DATEDIFF(Presidency_Start, Presidency_End) = (select max(DATEDIFF(Presidency_Start, Presidency_End)) from Presidents) Group by President_Name;')
    cursor.execute(query, )
    result = cursor.fetchone()
    cursor.close()
    conn.close()
...
def question_8():
    ...
    cursor = conn.cursor()
    query = (
    'select Party, count(President_Name) as temp from Presidents group by Party Having temp = count(President_Name);')
    cursor.execute(query, )
    result = cursor.fetchall()
    cursor.close()
    conn.close()
...
def question_10():
    ...
    cursor = conn.cursor()
    query = (
    'select State, count(State) as temp from Presidents group by State Having temp = count(State) order by temp desc limit 1;')
    cursor.execute(query, )
    result = cursor.fetchall()
    cursor.close()
    conn.close()
    print('The State that has produced the most presidents is: \n')
    print('Name: ', result[0][0], ' Number of presidents produced: ', result[0][1])
    print('#####################################################################')
...
def question_12(states):
    print('QUESTION 12')
    print(' ')
    conn = create_connection()
    cursor = conn.cursor()
    query = ('select distinct State from Presidents;')
    cursor.execute(query, )
    result = cursor.fetchall()
    cursor.close()
    conn.close()
    new_states = [items.strip('\xa0') for items in states]
    has_no_presidents = list(set([item for item in new_states]) - set([r[0] for r in result]))
    print('The following states have never produced a president')
    for items in has_no_presidents:
        print(items)
